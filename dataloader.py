...
class unetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 从文件中读取图像
        image = Image.open(r"./lgg-mri-segmentation/kaggle_3m/Images" + '/' + name + ".tif").convert('RGB')
        mask = Image.open(r"./lgg-mri-segmentation/kaggle_3m/Masks" + '/' + name + ".tif").convert('RGB')

        if self.random_data:
            image, mask = self.get_random_data(image,mask,(int(self.image_size[1]),int(self.image_size[0])))
        else:
            image, mask = letterbox_image(image, mask, (int(self.image_size[1]),int(self.image_size[0])))
