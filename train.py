...
#             print(labels.size())
            ...
            if aux_branch:
                ...
                loss1    = CE_Loss(aux, pngs, num_classes = NUM_CLASSES)
                loss=loss+loss1
                if dice_loss:
                    main_dice = Dice_loss(outputs, labels)
                    main_dice1 = Dice_loss(aux, labels)
                    loss      = loss + main_dice+main_dice1
        ...
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                if aux_branch:
                    ...
                    val_loss1 = CE_Loss(aux, pngs, num_classes = NUM_CLASSES)
                    val_loss=val_loss+val_loss1
                    if dice_loss:
                        main_dice = Dice_loss(outputs, labels)
                        main_dice1 = Dice_loss(aux, labels)
                        val_loss  = val_loss + main_dice+main_dice1
...
if __name__ == "__main__":
    ...
    # 打开数据集的txt
    with open(r"./lgg-mri-segmentation/kaggle_3m/ImageSets/Segmentation/val.txt","r") as f:
